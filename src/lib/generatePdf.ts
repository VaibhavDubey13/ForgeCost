...
export async function downloadPdfQuote(data: QuoteData): Promise<void> {
    ...
    content: [
      ...
      { canvas: [{ type: "rect", x: -48, y: -48, w: 800, h: 90, color: DARK }], absolutePosition: { x: 0, y: 0 } },
      {
        columns: [
          {
            stack: [
              { text: "MATCOST", fontSize: 26, bold: true, color: EMERALD },
      ...
      // ── Free tier watermark ───────────────────────────────────────────────
      ...(isFree ? [{
        text: "Generated with MatCost Free — upgrade to Pro to remove watermark",
        fontSize: 7,
        color: "#94a3b8",
      ...
      // Footer
      { text: `Generated by MatCost • ${data.date} • Quote valid for 30 days`, fontSize: 8, color: GRAY, alignment: "center", margin: [0, 32, 0, 8] },
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (pdfMake as any).createPdf(docDefinition).download(`matcost_${safeJob}_${data.date.replace(/\//g, "-")}.pdf`);
}